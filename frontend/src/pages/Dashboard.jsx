  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gold-500">Overview</h1>
        {!currentRate && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Metric Cards */}
        <div className="bg-dark-800 p-6 rounded-lg shadow-md border-l-4 border-gold-500">
          <h2 className="text-gold-400 text-sm uppercase tracking-wide">Current Gold Rate</h2>
        ...
        <div className="bg-dark-800 p-6 rounded-lg shadow-md border-l-4 border-gold-500">
          <h2 className="text-gold-400 text-sm uppercase tracking-wide">Total Gold Holdings</h2>
          <p className="text-3xl font-bold mt-2 text-gray-100">{totalWeightInGrams.toFixed(3)} <span className="text-lg text-gray-400">g</span></p>
          <span className="text-xs text-gray-400">{(totalWeightInGrams / 11.664).toFixed(2)} Tola</span>
        </div>

        <div className="bg-dark-800 p-6 rounded-lg shadow-md border-l-4 border-gold-500">
          <h2 className="text-gold-400 text-sm uppercase tracking-wide">Total Invested</h2>
          <p className="text-3xl font-bold mt-2 text-gray-100">Rs {Math.round(totalInvested).toLocaleString()}</p>
        </div>

        <div className={`bg-dark-800 p-6 rounded-lg shadow-md border-l-4 ${isProfit ? 'border-green-500' : 'border-red-500'}`}>
          <h2 className="text-gold-400 text-sm uppercase tracking-wide">Current Valuation</h2>
          <p className="text-3xl font-bold mt-2 text-gray-100">Rs {Math.round(currentValue).toLocaleString()}</p>
      ...
      {investments.length > 0 && (
        <div className="mt-8 bg-dark-800 p-6 rounded-lg shadow-md border border-gold-500/20">
          <h3 className="text-lg font-bold mb-4 text-gold-500">Investment Summary</h3>
          <p className="text-gray-300">Your portfolio is currently valued at <b className="text-gray-100">Rs {Math.round(currentValue).toLocaleString()}</b> against an investment of <b className="text-gray-100">Rs {Math.round(totalInvested).toLocaleString()}</b>.</p>
        </div>
      )}
    </div>
  );
